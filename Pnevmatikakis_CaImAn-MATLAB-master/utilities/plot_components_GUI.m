    ...
    function plot_component(i)
       if i <= nr
        ...
        if i <= nr
            ...
            if ~isempty(ff)
                [~,ww] = contour(reshape(A_temp,d1,d2),[0,0]+A_temp(ind(ff)),'LineColor','k','LineWidth',0.5);
                ww.LineWidth = 2;
            end
        ...
        subplot(3,2,[2,4,6]);
        if i <= nr
            plot(1:T,Y_r(i,:)/Df(i),'linewidth',2); hold all; plot(1:T,C(i,:)/Df(i),'linewidth',0.2);
            if plot_df
