function [CC,jsf,im] = plot_contours(Aor,Cn,ln_cl,pathPlot,options,display_numbers,max_number,Coor,ind_show,cm)
    ...
    if ~(nargin < 7 || isempty(Coor))
        ...
        for j = 1:length(ind_show)
            ...
            if size(Coor{i},2) > 1
                ...
                cont = [cont,cont(:,2)];
                plot(cont(1,2:end),cont(2,2:end),'Color',ln_cl, 'linewidth', 0.5); hold on;
            end            
        end
    else
	  ...
	  max_col = max(ln_cl(:,2));
        ...
        if strcmpi(options.thr_method,'nrg')
            ...
            for j = 1:size(Aor,2)
		if length(ln_cl) > 1
%  		  col = [j/size(Aor,2),0,1-j/size(Aor,2)];
		  i = ln_cl(j,1);
		  col = [ln_cl(j,2)/max_col,0,1-ln_cl(j,2)/max_col];
		else
		  i = j;
		  col = ln_cl;
		end
...
%                      CC{i} = contour(reshape(A_temp,d1,d2),[0,0]+A_temp(ind(ff)),'LineColor',ln_cl, 'linewidth', 2);
		    CC{i} = contour(reshape(A_temp,d1,d2),[0,0]+A_temp(ind(ff)),'LineColor',col, 'linewidth', 2);
...
%                          plot(BW2{ii}(:,1),BW2{ii}(:,2),'Color',ln_cl, 'linewidth', 0.5);
                        plot(BW2{ii}(:,1),BW2{ii}(:,2),'Color',col, 'linewidth', 0.5);
                    end
                    CC{i} = BW2{1}';
                    fp = find(BW);
                    [ii,jj] = ind2sub([d1,d2],fp);
