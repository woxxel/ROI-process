...
function patches = construct_patches(sizY,npatches,min_size)
...
% Changes made by A. Schmidt, 2018
...
for i = 1:sqrt(npatches)
  ...
  x_start(i) = max(1,(i-1)*patch_size(2)-overlap(2)/2);
  x_end(i) = min(sizY(2),i*patch_size(2)+overlap(2)/2);
  
  y_start(i) = max(1,(i-1)*patch_size(1)-overlap(1)/2);
  y_end(i) = min(sizY(1),i*patch_size(1)+overlap(1)/2);
  
  if dimY == 3
    z_start(i) = max(1,(i-1)*patch_size(3)-overlap(3)/2);
    z_end(i) = min(sizY(3),i*patch_size(3)+overlap(3)/2);
  else
    z_start = 1;
    z_end = 1;
  end
