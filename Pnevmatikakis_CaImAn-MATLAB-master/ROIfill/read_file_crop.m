function imData=read_file_crop(path_to_file,extents,options)
...
%     name = info.GroupHierarchy.Datasets.Name;
    ...
    offset = [extents(1,1),extents(2,1),sframe];
    span = [extents(1,2)-extents(1,1)+1,extents(2,2)-extents(2,1)+1,num2read];
    
%    imData = h5read(path_to_file,name,[ones(1,length(dims)-1),sframe],[dims(1:end-1),num2read]);
    imData = h5read(path_to_file,['/',name],offset,span);    
