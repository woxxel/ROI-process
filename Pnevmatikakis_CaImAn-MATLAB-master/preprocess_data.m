...
if options.flag_g && (~isfield(options,'pixels') || isempty(options.pixels))
    options.pixels = 1:numel(Y)/size(Y,ndims(Y));
end
...
if options.flag_g
    ...
    if split_data
        Ycl = mat2cell(Y_tmp(ff,:),ones(np,1),size(Y_tmp,2));
        XC = cell(np,1);
        parfor j = 1:np
        ...
        parfor j = 1:np
          XC(j,:) = xcov(Y_tmp(ff(j),:),lags,'biased');
        end
    end
