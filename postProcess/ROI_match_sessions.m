...
function ROI_match_sessions(pathMouse,parameter)
    
    tic
    [sessionList, nSessions] = getSessions(pathMouse);
    pathShift = dir(pathcat(pathMouse,'all_shift*.txt'));
    pathResults = pathcat(pathMouse,'All_ROIs.mat');
...
%      end
    ...
    for s = 1:nSessions
        loadDat = load(pathcat(sessionList{s},'ROIs.mat'),'ROIs');
        ...
        for n = ncells_session:-1:1 % start with last, to allow for preallocation of structures
            ...
            loadROIs(n) = ROI_update(loadROIs(n),'truncate');       % truncate ROIs, where they overlap borders and recalculate norm
            ...
            for m = n+1:ncells
                ...
                if m > n_border   % remove iterating over same-session
                    ...
                    if tmp_dist <= max_dist
                        ...
                        tmp_corr = ROIs_dotproduct(ROIs(n),ROIs(m),'norm')/(ROIs(n).norm*ROIs(m).norm);
                        if tmp_corr > 0
...
%      histo.corr(:,2) = histo.corr(:,2)/sum(histo.corr(:,2));
    ...
    saveas(fig_dist,pathcat(pathMouse,sprintf('matchHistoDist_d=%4.2f.jpg',parameter.max_dist)),'jpg')
    ...
    legend('nearest neighbour','others')
    title(sprintf('max dist = %4.2f',parameter.max_dist))
    saveas(fig_corr,pathcat(pathMouse,sprintf('matchHistoCorr_d=%4.2f.jpg',parameter.max_dist)),'jpg')
