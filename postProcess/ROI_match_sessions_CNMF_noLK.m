...
function ROI_match_sessions_CNMF_noLK(pathMouse,parameter)
    ...
    pathHisto = pathcat(pathMouse,sprintf('matchSessions_bin_d=%4.2f_noLK.mat',parameter.max_dist));
...
%      end
    ...
    for s = 1:nSessions
        loadDat = load(pathcat(sessionList{s},'ROIs_CNMF_noLK.mat'),'ROIs');
...
%      histo.corr(:,2) = histo.corr(:,2)/sum(histo.corr(:,2));
    ...
    saveas(fig_dist,pathcat(pathMouse,sprintf('matchHistoDist_d=%4.2f_nSes=%d_noLK.jpg',parameter.max_dist,nSessions)),'jpg')
    ...
    legend('nearest neighbour','others')
    title(sprintf('max dist = %4.2f',parameter.max_dist))
    saveas(fig_corr,pathcat(pathMouse,sprintf('matchHistoCorr_d=%4.2f_nSes=%d_noLK.jpg',parameter.max_dist,nSessions)),'jpg')
